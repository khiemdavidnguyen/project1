...
public class ManagerSubmitRequestServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session == null)
			...
			request.getRequestDispatcher("WebPages/html/manager-submit-request.html").forward(request, response);
	}
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
			...
			if(rd.createRequest( new Reimbursement (0, amount, strDate,  null , description, receipt, staffId, 0, 1, type, null, null ) ) == 1)
				response.sendRedirect("manager-success");
		}
	}
}
